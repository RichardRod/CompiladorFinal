package lexico;


public class TipoSimbolo {

    public static final int ERROR = -1;
    public static final int IDENTIFICADOR = 0;
    public static final int ENTERO = 1;
    public static final int REAL = 222;
    public static final int CADENA = 333;
    public static final int TIPO = 444;
    public static final int opSUMA = 3;
    public static final int opMULT = 2;
    public static final int opRELAC = 7;
    public static final int opOR = 8;
    public static final int opAND = 9;
    public static final int opNOT = 10;
    public static final int opIGUALDAD = 11;
    public static final int PUNTO_COMA = 12;
    public static final int COMA = 13;
    public static final int PARENTESIS_INICIO = 14;
    public static final int PARENTESIS_FIN = 15;
    public static final int LLAVE_INICIO = 16;
    public static final int LLAVE_FIN = 17;
    public static final int IGUAL = 18;
    public static final int IF = 19;
    public static final int WHILE = 20;
    public static final int RETURN = 21;
    public static final int ELSE = 22;
    public static final int PESOS = 4;
}
